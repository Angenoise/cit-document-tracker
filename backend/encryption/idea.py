...
class IDEA:
    ...
    # IDEA works on 64-bit blocks, so each block is 8 bytes long.
    BLOCK_SIZE = 8  # 64 bits
    # The user key is 128 bits, which is 16 bytes.
    KEY_SIZE = 16   # 128 bits
    # IDEA always uses 8 main rounds.
    ROUNDS = 8
    # IDEA needs 52 subkeys in total.
    TOTAL_SUBKEYS = 52  # 6 per round + 4 for final transformation
    
    # IDEA uses modular multiplication with the special modulus 65537.
    MOD_PRIME = 65537  # 2^16 + 1
    
    def __init__(self, key: bytes):
        """
        Initialize IDEA cipher with a 128-bit key
        ...
        self.key = key
        # Build the 52 subkeys once so the cipher can reuse them for each block.
        self.subkeys = self._generate_subkeys()
    
    @staticmethod
    def _multiplicative_inverse(x: int) -> int:
        """
        Calculate multiplicative inverse modulo 2^16 + 1
        Returns 0 if x is 0, 1 if x is 1
        """
    ...
    def _generate_subkeys(self) -> List[int]:
        """
        Generate 52 subkeys from the 128-bit key
        6 subkeys per round (8 rounds) + 4 for final transformation = 52 total
        ...
        # IDEA expands the key by repeatedly rotating and reusing bits.
    ...
    def _modular_add(a: int, b: int) -> int:
        ...
        # The & mask keeps the result inside 16 bits.
        return (a + b) & 0xFFFF
    ...
    def _round_function(self, x1: int, x2: int, x3: int, x4: int, 
        ...
        """
        # Each round consumes six subkeys.
        k1, k2, k3, k4, k5, k6 = (
...
def encrypt_document_id(document_id: str, key: bytes) -> str:
    """
    Encrypt a document ID and return as hexadecimal string
...
def decrypt_document_id(encrypted_hex: str, key: bytes) -> str:
    """
    Decrypt an encrypted document ID from hexadecimal string
